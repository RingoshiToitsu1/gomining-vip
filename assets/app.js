...
// Repriced 2026-07-30 (cut ~9.5%), then +11.77% GoMining-wide on 2026-09-08. All twenty tiers
// re-observed 2026-09-08 against a 1 TH 15 W miner and written here exactly, so nothing on this
// curve is extrapolated. The flat +11.77% estimate it replaced was already within a cent on
// 14 of 20 tiers, which is why the two agree so closely.
// Quotes are net of the 5% NFT discount and stored grossed up by /0.95 — avatarDiscMult()
// applies that discount separately, so storing the net figure would take it twice.
...
function openPlannerForm(){
  ...
  if($('piRefPriorTH')&&$('inRefPriorTH'))$('piRefPriorTH').value=$('inRefPriorTH').value;
...
function submitPlannerCapital(){
  ...
  setTimeout(function(){
   try{
    ...
    if($('piRefPriorTH')&&$('inRefPriorTH'))$('inRefPriorTH').value=parseFloat($('piRefPriorTH').value)||0;
...
function drawCombined(){
  ...
  if(_cmbStyle==='candle'){
    // Wicks reach past the closes the lines are drawn from; fit BOTH assets' or they clip.
    [['btcC',S[0].btc],['gmtC',S[0].gmt]].forEach(([key,base])=>{
  ...
  if(_cmbStyle==='candle'){
    // Both assets, paired inside each day: Bitcoin on the left half of the slot, GMT on the
    // right. Overlaying them would hide whichever drew second; side by side you can read both
    // days at a glance and the pairing itself shows where they diverged.
    const slot=pw/Math.max(1,S.length);
    const half=slot/2;
    const bw=Math.max(1,Math.min(9,half*0.72));
    x.save();x.beginPath();x.rect(L,T,pw,ph);x.clip();
    const draw=(key,base,upC,dnC,offset)=>{
      // Re-based on the SAME divisor as that asset's line, so switching style never moves the
      // series and the axis keeps meaning "% change since the left edge".
      const Yv=v=>Y(v/base*100);
      S.forEach((p,ix)=>{
        const k=p[key];if(!k)return;
        const cx=L+ix*slot+offset;
        const col=(k.c>=k.o)?upC:dnC;
        x.strokeStyle=col;x.fillStyle=col;x.lineWidth=Math.max(1,half*0.16);
    ...
    draw('btcC',S[0].btc,CMB_BTC_UP,CMB_BTC_DN,half*0.5);
    draw('gmtC',S[0].gmt,CMB_GMT_UP,CMB_GMT_DN,half*1.5);
    x.restore();
  }else{
    line('biR',CMB_BTC,2);
    line('giR',CMB_GMT,2);
      ...
      if(tip){
        ...
        tip.innerHTML=`<div class="rb-tip-date">${new Date(p.t).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric',timeZone:'UTC'})}</div>`
          ...
          +(function(){
            ...
            let out='';
            [['BTC',p.btcC,v=>fmtBTCPrice(v)],['GMT',p.gmtC,v=>'$'+v.toFixed(4)]].forEach(([nm,k,q],ix)=>{
              if(!k)return;
              out+=`<div class="rb-tip-row"${ix===0?' style="margin-top:.2rem;border-top:1px solid var(--border);padding-top:.25rem"':''}><span class="rb-tip-lbl">${nm} O/C</span><span class="rb-tip-px">${q(k.o)} / ${q(k.c)}</span></div>`
  ...
  const sw=(a,b)=>`<span style="display:inline-block;width:7px;height:7px;border-radius:2px;background:${a};margin-right:2px"></span><span style="display:inline-block;width:7px;height:7px;border-radius:2px;background:${b};margin-right:5px"></span>`;
  const cndl=_cmbStyle==='candle';
  if(lg)lg.innerHTML=
     `<span class="rb-pill" style="border-left-color:${CMB_BTC}">${cndl?sw(CMB_BTC_UP,CMB_BTC_DN):''}BTC ${pct(last.biR)} &middot; ${fmtBTCPrice(last.btc)}</span>`
    +`<span class="rb-pill" style="border-left-color:${CMB_GMT}">${cndl?sw(CMB_GMT_UP,CMB_GMT_DN):''}GMT ${pct(last.giR)} &middot; $${last.gmt.toFixed(4)}</span>`
...
const SHARE_FIELDS=[
  ...
  ['inReferredTH'],['inRefCapital'],['inCurrency'],['piVipBonus','b'],['inGreedyWth'],['inMpGreedy','b'],['inMpCode'],['inRefPriorTH']
];
...
function inp(){
  ...
  // TH this referral already owns from earlier purchases with your code. Distinct from `refTH`
  // (the Ambassador field), which is a static 15 W lump used only for ambassador income: this
  // joins the SAME fleet the planned capital buys, so it earns, reinvests and pays commission.
  refPriorTH:+($('inRefPriorTH')?$('inRefPriorTH').value:0)||0
}}
...
const MINING_MODE_DEFAULT=1.35;   // observed 2026-09-25
...
const GREEDY_GROWTH_DEFAULT='0.36';   // observed 2026-09-25
const GREEDY_GROWTH_PAST_DEFAULTS=['0.3','0.30','0.35','0.3718','0.3462'];
...
const MINING_MODE_PAST_DEFAULTS=[0.7,0.82,0.83,0.88,0.9,1.06,1.2,1.28,1.36];
...
function readInputs(){
  return {
    ...
    inRefPriorTH:$('inRefPriorTH')?$('inRefPriorTH').value:'0',
...
function applyInputs(d){
  ...
  if(d.inRefPriorTH!=null&&$('inRefPriorTH'))$('inRefPriorTH').value=d.inRefPriorTH;
...
['inTH','inWTH','inGMTLocked','inGMTWallet','inCapital','inReferredTH','inRefCapital','inRefBonusPct','inRefReinvest','inMpTH','inMpGMT','inMpWth','inGreedyTH','inGreedyInitial','inGreedyWth','inGreedyGrowth','inMpCode','inRefPriorTH'].forEach(id=>{const e=$(id);if(e)e.addEventListener('input',autoSave)});
...
function renderIdleGmt(i,m){
  ...
  // `act` is the instruction — the TH to buy, the GMT to lock. It leads each leg, because it is
  // the only part the reader has to carry to the GoMining app; the % and $ are context for the
  // split and were being shown louder than the number they explain.
  const legs=[
    {k:'Lock GMT',v:P.lockUSD,
     act:P.glAdd>0.5?fN(P.glAdd,0)+' GMT':null,sub:''},
    {k:'Buy TH',v:P.thUSD,
     act:P.addTH>0.5?'+'+fN(P.addTH,1)+' TH':null,
     sub:(P.addTH>0.5&&fs)?`at ${wLbl(fs.wth)} W/TH${fs.mixed?' avg':''}`:'',
     into:(P.addTH>0.5&&fs)?`${fs.name}${fs.more>0?` +${fs.more} more`:''}`:''}
  ];
  if(P.effRoom)legs.push({k:'Upgrade Efficiency',v:P.effUSD,
    act:P.effTHupg>0.5?fN(P.effTHupg,0)+' TH \u2192 12 W':null,sub:'',
  ...
  let h=`<div class="idle-gmt">
    ...
    <div class="idle-gmt-legs">${legs.map((l,n)=>`<div class="idle-gmt-leg${l.act?'':' is-none'}${l.act?' has-pulse':''}"><span class="idle-gmt-dot s${n}"></span><div><div class="idle-gmt-leg-k">${l.k}</div>`
      +(l.act?`<div class="idle-gmt-leg-act"><span>${l.act}</span></div>`:`<div class="idle-gmt-leg-act none">nothing</div>`)
...
function renderPlanner(i,m){
  ...
  if(ref){
    ...
    ph+=row('Referral TH',`${fN(refInitTH,1)} TH<span class="sub">${fU(ref.thUSD)} invested${refPriorTH>0?` &middot; on top of ${fN(refPriorTH,0)} TH they already own`:''}</span>`,'cyan');
...
function computeSetupProjection(){
  ...
  if(fromPlanner){
    ...
    // Their earlier purchases are part of the same farm, so they earn and compound with it.
    // Their existing TH also brings coverage at the SAME ratio the new purchase is funded at.
    // Adding hashrate with no GMT behind it would be a farm nobody runs — and it back-fires:
    // the larger fee bill drains their weekly rewards into coverage instead of TH, which is
    // paid commission, so declaring their real fleet would have CUT your projected commission.
    const _prior=Math.max(0,i.refPriorTH||0);
    if(_prior>0){
    ...
    MP_TH=0; MP_WTH=15;
    // Their existing fleet compounds with or without a new purchase. With no purchase to take a
    // coverage ratio from, seed it at the 360-day coverage this app treats as the target.
    refFleetTH=Math.max(0,i.refPriorTH||0);
    if(refFleetTH>0){
  ...
  function dailyNet(curTH,curLocked,ov){
    ...
    // The greedy machine's free weekly TH, valued as income — the same treatment the console
    // hero and the Capital Planner give it, so all three headline numbers now mean one thing.
    // It is NOT double counting: this is income received in the form of hashrate, exactly like
    // mined BTC that gets reinvested, and the resulting TH earns on top from then on. It is
    // deliberately kept OUT of `reinvest` — the value arrived already deployed as TH, so it must
    // never be handed to the allocator to spend a second time.
    // 4.33/30 (not /7) so a day x30 lands on the planner's weekly x 4.33 monthly figure exactly.
    const greedyUSD=(gTH>0&&GGROW>0)?(gTH*GGROW)*4.33/30*cptAtEff(gTH,gW):0;
    return{net:miningUSD+stakingUSD+ambDaily+greedyUSD,reinvest:miningUSD+ambDaily,
      mining:miningUSD,staking:stakingUSD,amb:ambDaily,greedy:greedyUSD,disc:fd,tokD:td,vip:v.n};
  }
  ...
  const breakdownMonthly=`mining ${fU(fb.mining*30)} + staking ${fU(fb.staking*30)}${ambDaily>0?` + ambassador ${fU(fb.amb*30)}`:''}${fb.greedy>0?` + greedy growth ${fU(fb.greedy*30)}`:''}`;
